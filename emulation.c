...
void handle_interrupts(CPU* cpu) {
    ...
    uint8_t IE = read_byte(0xFFFF); // Interrupt Enable
    uint8_t IF = read_byte(0xFF0F); // Interrupt Flag
...
int emulate_cycle(CPU* cpu) {
    ...
    uint8_t ie    = read_byte(0xFFFF); // Interrupt Enable
    uint8_t iflag = read_byte(0xFF0F); // Interrupt Flag
    ...
    switch (opcode) {
        ...
        case 0x01: { // LD BC, d16. Copies value from BC to d16
            load_r16_n16(&cpu->bc.BC, cpu);
            break;
        }
        case 0x02: { // LD (BC), A. Stores A into the at address BC
            load_p16_r8(&cpu->bc.BC, &cpu->af.A, cpu);
            break;
        }
        case 0x03: { // INC BC. Increments BC by 1
            inc_r16(&cpu->bc.BC, cpu);
            break;
        }
        ...
        case 0x39:{// ADD HL, SP. Sum registers HL and SP
            add_r16_r16(&cpu->hl.HL, &cpu->sp, cpu);
            break;
        }
        case 0x3A:{// LD A, [HL-]. Copy the value pointed by HL to A and rest 1
            load_r8_p16_minus(&cpu->af.A, &cpu->hl.HL, cpu);
            break;
        }
        ...
        case 0x40:{// LD B, B. copy B into B?
            load_r8_r8(&cpu->bc.B, &cpu->bc.B, cpu);
            break;
        }
        case 0x41:{// LD B, C . Copy C into B
            load_r8_r8(&cpu->bc.B, &cpu->bc.C, cpu);
            break;
        }
        case 0x42:{// LD B, D . Copy D into B
            load_r8_r8(&cpu->bc.B, &cpu->de.D, cpu);
            break;
        }
        case 0x43:{// LD B, E . Copy E into B
            load_r8_r8(&cpu->bc.B, &cpu->de.E, cpu);
            break;
        }
        case 0x44:{// LD B, H . Copy H into B
            load_r8_r8(&cpu->bc.B, &cpu->hl.H, cpu);
            break;
        }
        case 0x45:{// LD B, L . Copy L into B
            load_r8_r8(&cpu->bc.B, &cpu->hl.L, cpu);
            break;
        }
        case 0x46:{// LD B, [HL] . Copy direction pointed by HL into B
            load_r8_p16(&cpu->bc.B, &cpu->hl.HL, cpu);
            break;
        }
        case 0x47:{// LD B, A . Copy A into B
            load_r8_r8(&cpu->bc.B, &cpu->af.A, cpu);
            break;
        }
        case 0x48:{// LD C, B . Copy B into C
            load_r8_r8(&cpu->bc.C, &cpu->bc.B, cpu);
            break;
        }
        case 0x49:{// LD C, C . Copy C into C?
            load_r8_r8(&cpu->bc.C, &cpu->bc.C, cpu);
            break;
        }
        case 0x4A:{// LD C, D . Copy D into C
            load_r8_r8(&cpu->bc.C, &cpu->de.D, cpu);
            break;
        }
        case 0x4B:{// LD C, E . Copy E into C
            load_r8_r8(&cpu->bc.C, &cpu->de.E, cpu);
            break;
        }
        case 0x4C:{// LD C, H . Copy H into C
            load_r8_r8(&cpu->bc.C, &cpu->hl.H, cpu);
            break;
        }
        ...
        case 0x4E:{// LD C, [HL] . Copy direction pointed in HL into C
            load_r8_p16(&cpu->bc.C, &cpu->hl.HL, cpu);
            break;
        }
        case 0x4F:{// LD C, A . Copy A into C
            load_r8_r8(&cpu->bc.C, &cpu->af.A, cpu);
            break;
        }
        case 0x50:{// LD D, B . Copy B into D
            load_r8_r8(&cpu->de.D, &cpu->bc.B, cpu);
            break;
        }
        case 0x51:{// LD D, C . Copy C into D
            load_r8_r8(&cpu->de.D, &cpu->bc.C, cpu);
            break;
        }
        case 0x52:{// LD D, D . Copy D into D?
            load_r8_r8(&cpu->de.D, &cpu->de.D, cpu);
            break;
        }
        case 0x53:{// LD D, E . Copy E into D
            load_r8_r8(&cpu->de.D, &cpu->de.E, cpu);
            break;
        }
        case 0x54:{// LD D, H . Copy H into D
            load_r8_r8(&cpu->de.D, &cpu->hl.H, cpu);
            break;
        }
        case 0x55:{// LD D, L . Copy L into D
            load_r8_r8(&cpu->de.D, &cpu->hl.L, cpu);
            break;
        }
        case 0x56:{// LD D, [HL] . Copy value pointed by HL into D
            load_r8_p16(&cpu->de.D, &cpu->hl.HL, cpu);
            break;
        }
        case 0x57:{// LD D, A . Copy A into D
            load_r8_r8(&cpu->de.D, &cpu->af.A, cpu);
            break;
        }
        case 0x58:{// LD E, B . Copy B into E
            load_r8_r8(&cpu->de.E, &cpu->bc.B, cpu);
            break;
        }
        case 0x59:{// LD E, C . Copy C into E
            load_r8_r8(&cpu->de.E, &cpu->bc.C, cpu);
            break;
        }
        case 0x5A:{// LD E, D . Copy D into E
            load_r8_r8(&cpu->de.E, &cpu->de.D, cpu);
            break;
        }
        case 0x5B:{// LD E, E. Copy E into E
            load_r8_r8(&cpu->de.E, &cpu->de.E, cpu);
            break;
        }
        case 0x5C:{// LD E, H. Copy H into E
            load_r8_r8(&cpu->hl.H, &cpu->de.E, cpu);
            break;
        }
        case 0x5D:{// LD E, L. Copy L into E
            load_r8_r8(&cpu->hl.L, &cpu->de.E, cpu);
            break;
        }
        case 0x5E:{// LD E, [HL]. Copy value pointed by HL into E
            load_r8_p16(&cpu->de.E, &cpu->hl.HL, cpu);
            break;
        }
        case 0x5F:{// LD E, A. Copy A into E.
            load_r8_r8(&cpu->de.E, &cpu->af.A, cpu);
            break;
        }
        case 0x60:{// LD H, B. Copy B into H.
            load_r8_r8(&cpu->hl.H, &cpu->bc.B, cpu);
            break;
        }
        case 0x61:{// LD H, C . Copy C into H
            load_r8_r8(&cpu->hl.H, &cpu->bc.C, cpu);
            break;
        }
        case 0x62:{// LD H, D. Copy D into H.
            load_r8_r8(&cpu->hl.H, &cpu->de.D, cpu);
            break;
        }
        ...
        case 0x64:{// LD H, H. Copy H into H?
            load_r8_r8(&cpu->hl.H, &cpu->hl.H, cpu);
            break;
        }
        case 0x65:{// LD H, L. Copy L into H
            load_r8_r8(&cpu->hl.L, &cpu->hl.H, cpu);
            break;
        }
        case 0x66:{// LD H, [HL]. Copy value pointed in HL into H
            load_r8_p16(&cpu->hl.L, &cpu->hl.HL, cpu);
            break;
        }
        ...
        case 0x68:{// LD L, B. Copy B into L
            load_r8_r8(&cpu->hl.L, &cpu->bc.B, cpu);
            break;
        }
        case 0x69:{// LD L, C. Copy C into L
            load_r8_r8(&cpu->hl.L, &cpu->bc.C, cpu);
            break;
        }
        case 0x6A:{//  LD L, D. Copy D into L
            load_r8_r8(&cpu->hl.L, &cpu->de.D, cpu);
            break;
        }
        ...
        case 0x6D:{// LD L, L. Copy L into L?
            load_r8_r8(&cpu->hl.L, &cpu->hl.L, cpu);
            break;
        }
        ...
        case 0x77:{// LD [HL], A. Copy A into value pointed by HL.
            load_p16_r8(&cpu->hl.HL, &cpu->af.A, cpu);
            break;
        }
        case 0x78:{// LD A, B. Copy B into A.
            load_r8_r8(&cpu->af.A, &cpu->bc.B, cpu);
            break;
        }
        case 0x79:{// LD A, C. Copy C into A.
            load_r8_r8(&cpu->af.A, &cpu->bc.C, cpu);
            break;
        }
        case 0x7A:{// LD A, D. Copy D into A.
            load_r8_r8(&cpu->af.A, &cpu->de.D, cpu);
            break;
        }
        case 0x7B:{// LD A, E. Copy E into A.
            load_r8_r8(&cpu->af.A, &cpu->de.E, cpu);
            break;
        }
        case 0x7C:{// LD A, H. Copy H into A.
            load_r8_r8(&cpu->af.A, &cpu->hl.H, cpu);
            break;
        }
        case 0x7D:{// LD A, L. Copy L into A.
            load_r8_r8(&cpu->af.A, &cpu->hl.L, cpu);
            break;
        }
        ...
        case 0xC5:{// PUSH BC. Copy BC to stack.
            push_r16(&cpu->bc.BC, cpu);
            break;
        }
        case 0xC6:{// ADD A, n8. Sum register A and n8
            add_r8_n8(&cpu->af.A, cpu);
            break;
        }
        ...
        case 0xD5:{// PUSH DE. Save current pc into stack
            push_r16(&cpu->de.DE, cpu);
            break;
        }
        case 0xD6:{// SUB A, n8. Substract n8 to A
            sub_r8_n8(&cpu->af.A, cpu);
            break;
        }
        case 0xD7:{// RST $10. Save current to stack and go to 0x10
            rst_vec(0x10, cpu);
            break;
        }
        ...
        case 0xD9:{// RETI. Interruptions stuff
            cpu->pc = read_byte(cpu->sp) | (read_byte(cpu->sp + 1) << 8);
            cpu->sp += 2;
        ...
        case 0xE0:{// LDH [a8], A. Copy value in A into a8
            load_a8_r8(&cpu->af.A, cpu);
            break;
        }
        case 0xE1:{// POP HL. Save stack into HL
            pop_r16(&cpu->hl.HL, cpu);
            break;
        }
        case 0xE2:{// LDH [C], A. Copy A into C
            load_c_r8(&cpu->bc.C, cpu);
            break;
        }
